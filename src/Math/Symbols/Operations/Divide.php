...
class Divide extends Operation {
	...
	public function __construct() {
		// a / b --> (new Divide())->over(a)->with(b)->assign([a => 4, b => 3.2])->perform() --> 1.25
	}

	public function __toString(): string {
		if ($this->is_function) return "divide({$this->over['a']}, {$this->with['b']})";
	...
	public function evaluate() {
		...
		if (!method_exists($a, 'divide')) {
			throw new \Error('Operand object ‹‹'.Delegator::getType($a).'›› does not support ‹‹divide()›› method');
		}
		$result = $a->divide($b);
		if (is_null($result)) {
